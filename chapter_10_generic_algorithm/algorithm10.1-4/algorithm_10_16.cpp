...
void biggies(vector<string> vec,vector<string>::size_type sz){
   ...
   auto sz_begin=find_if(vec.begin(),vec.end(),[sz](const string &s1)->bool {return s1.size()>=sz;});
   auto len=vec.end()-sz_begin;
   cout<<len<<endl;
   for_each(sz_begin,vec.end(),[](const string &s1){cout<<s1<<" ";});
   cout<<endl;
}
int main(){
   vector<string> vec;
